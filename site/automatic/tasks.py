...
def resetlikes():
    ...
    for profile in profiles:
        if profile.limit == 0:
            if profile.tg_id:
                send_msgtg(profile.tg_id, "Теперь снова можно ставить лайки!\n"
                                          "Поищем пару?")
            else:
                send_msgvk(profile.vk_id, "Теперь снова можно ставить лайки!\n"
                                          "Поищем пару?")
        if profile.limit < 30:
            profile.limit = 30
            profile.save()
...
@app.task
def register_user(pr_id):
    print(pr_id)
    new_user = Profile.objects.filter(id=pr_id)
    User.objects.create(id=new_user.id,
                        username=new_user.id,
                        password='pbkdf2_sha256$600000$DFfLyopttK7X4p8yIX4bWD$7OJmpmDdP1LPxveVoPgD/4X1+Hac7SaP+Iwx/83r9wo=')
    new_user.user = pr_id
    new_user.save()
# nha4RZ